...
const CorrelationMatrix = ({ assets, matrix, onSelectAsset }) => (
                ...
                {matrix[rowIndex].map((value, colIndex) => {
                    ...
                    return (
                        <button
                          ...
                          type="button"
                          onClick={() => onSelectAsset(assets[rowIndex], assets[colIndex], value)}
                          className={`group relative flex h-12 items-center justify-center rounded-md ${bgColor} transition-transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-400`}
                        >
...
const CorrelationDashboardPage = () => {
  ...
  const handleAssetSelect = (primaryAsset, secondaryAsset, correlation) => {
    if (!primaryAsset) return;
    const symbol = primaryAsset.replace('/', '').toUpperCase();
    ...
    <div className="min-h-screen bg-primary-bg p-4 sm:p-6 lg:p-8 text-text-primary">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="overflow-x-auto rounded-lg bg-card-bg p-4">
            <CorrelationMatrix assets={assets} matrix={matrix} onSelectAsset={handleAssetSelect} />
        </div>
      </div>
    </div>
  );
