...
import { Search, AlertCircle, Zap, TrendingUp } from 'lucide-react';
import { Smile, Frown, Meh } from 'lucide-react';
import { mockNews } from '@/data/newsData';
import { useNavigate } from 'react-router-dom';
import RealTimeNewsTicker from '@/components/news/RealTimeNewsTicker';
import InstrumentNewsPanel from '@/components/news/InstrumentNewsPanel';
...
const NewsCard = ({ item, onClick, onSymbolClick }) => (
    <motion.div
        whileHover={{ y: -4 }}
            ...
            <div className="flex items-center gap-2">
                {item.symbols.map(s => (
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      onSymbolClick && onSymbolClick(s);
                    }}
                    className="bg-primary-bg text-text-secondary text-xs px-2 py-1 rounded hover:bg-accent-orange/10 hover:text-accent-orange transition-colors"
                  >
                    #{s}
                  </button>
                ))}
...
const NewsHub = () => {
    ...
    const [showInstrumentNews, setShowInstrumentNews] = useState(false);
  ...
  return (
    <>
      ...
      <div className="p-4 sm:p-6 lg:p-8" style={{ marginTop: showTicker ? '60px' : '0' }}>
        <Helmet>
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 bg-accent-blue/10 text-accent-blue rounded-lg text-sm font-medium hover:bg-accent-blue/20 transition-colors"
                >
                  <Zap size={16} />
                  Instrument News
                </button>
            ...
            <p className="text-base text-text-secondary mt-1">AI-powered financial news analysis with real-time updates</p>
        </motion.div>
      ...
      {/* Instrument News Panel */}
      <InstrumentNewsPanel
        symbol={selectedSymbol}
        isOpen={showInstrumentNews}
        onClose={() => setShowInstrumentNews(false)}
      />
    </div>
    </>
  );
