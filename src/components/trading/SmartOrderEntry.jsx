...
const SmartOrderEntry = ({ asset }) => {
  ...
  const [price, setPrice] = useState(asset ? asset.price : '0.0000');
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        <button onClick={() => setSide('buy')} className={`py-2 rounded-md font-semibold transition-colors ${side === 'buy' ? 'bg-green-600 text-white' : 'bg-white/5 hover:bg-white/10'}`}>BUY</button>
        <button onClick={() => setSide('sell')} className={`py-2 rounded-md font-semibold transition-colors ${side === 'sell' ? 'bg-red-600 text-white' : 'bg-white/5 hover:bg-white/10'}`}>SELL</button>
      </div>
      ...
      <div className="text-sm text-text-secondary mb-2">Price: <span className="font-mono text-text-primary">{price}</span></div>
      ...
      <AlertDialog open={isConfirming} onOpenChange={setIsConfirming}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to place a {side} order for {amount || calculateAmountFromRisk()} {asset.symbol} at market price?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
